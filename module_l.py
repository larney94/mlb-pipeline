...
• Inputs:
    ...
    - true_lines_<DATE>.csv (from outputs.dir/true_lines)
    - Jinja2 template (from llm.prompt_template)
• Outputs:
    - outputs.dir/mlb_llm_predictions_<DATE>.csv
        Columns: player, model_pred, line, llm_pred, flag, confidence, prompt_version
    - outputs.dir/mlb_llm_explanations_<DATE>.txt
        One line per player — natural language LLM-generated output
• Config keys: llm.*, outputs.dir, overwrite_policy, logging.*
...
def main():
    ...
    overwrite = args.overwrite_policy or config.get("overwrite_policy", "error")
    ...
    lines_path = Path(config["outputs"]["dir"]) / "true_lines" / f"true_lines_{date_str}.csv"
    prompt_path = Path(config["llm"]["prompt_template"]).resolve()
    endpoint = config["llm"]["endpoint"]

    output_csv = resolve_output_path(Path(config["outputs"]["dir"]) / f"mlb_llm_predictions_{date_str}.csv", overwrite)
    output_txt = resolve_output_path(Path(config["outputs"]["dir"]) / f"mlb_llm_explanations_{date_str}.txt", overwrite)
