...
def main():
    ...
    parser.add_argument("--set", nargs="*", default=[], help="Override config values with dotted keys")

    args = parser.parse_args()
    config = load_config(args.config_path)
    config = apply_cli_overrides(config, args.set)  # ✅ Fixed: args → args.set

    logger = get_rotating_logger("module_a", log_dir="logs")
    logger.info(f"Loaded config: {config}")

    seasons = [args.season] if args.season else config.inputs.seasons
    output_dir = Path(config.outputs.gamelogs_dir)
    timeout = getattr(config, "pybaseball_timeout", 10)
    overwrite_policy = args.overwrite_policy or config.overwrite_policy
    ...
    for season in seasons:
        ...
        batting_path = resolve_output_path(output_dir / f"mlb_all_batting_gamelogs_{season}.csv", overwrite_policy)
        pitching_path = resolve_output_path(output_dir / f"mlb_all_pitching_gamelogs_{season}.csv", overwrite_policy)
