...
# --------------------------------------------------------------------------- #
# 🔑  Pydantic-safe CLI override merge (corrected version)
# --------------------------------------------------------------------------- #
def apply_cli_overrides(cfg: BaseModel | dict, overrides: Sequence[str] | None = None) -> BaseModel | dict:
    """
    ...
    * Returns the **same type** as provided (BaseModel or dict).
    ...
    if hasattr(cfg, "model_dump"):  # Pydantic v2
        ...
        raw_dict = deepcopy(cfg)  # already a dict
    ...
    for raw in overrides:
        ...
        _set_dotted(raw_dict, dotted_key, parsed_val)

    return cfg.__class__(**raw_dict) if not isinstance(cfg, dict) else raw_dict
